...
OUTPUT_FILE = Path("ml/data/full_features.csv")
...
for case_dir in sorted(BASE_DIR.iterdir()):
    ...
    for i in range(max_cells):
        ...
        risky_label = int(source_changed == 1 or execution_changed == 1)

        rows.append({
            "case_name": case_name,
            "cell_index": i + 1,
            "execution_count": execution_count,
            "fixed_execution_count": fixed_execution_count,
            ...
            "has_file_read": int(
                "read_csv" in source_lower
                or "read_excel" in source_lower
                or "open(" in source_lower
            ),
